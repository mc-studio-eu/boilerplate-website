...
const scrollToPrincipe = () => {
  const el = document.getElementById('principe')
  if (el) {
...
<template>
  <section
    class="relative w-full rounded-lg overflow-hidden min-h-[calc(80svh-10px)] sm:min-h-[520px] lg:min-h-[580px] mb-14"
  >
    <!-- Background Image -->
    <NuxtImg
    ...
    <!-- Content -->
    <div
      class="relative z-10 flex flex-col h-full px-4 sm:px-6 pt-6 sm:pt-8 pb-10 sm:pb-16 max-w-[1180px] mx-auto"
    >
      <Navbar />

      <div class="flex flex-1 flex-col items-start justify-center gap-8 mt-8 sm:mt-10 max-w-[720px]">
        <div
        ...
        >
          ...
          <span class="font-inter text-[11px] uppercase tracking-[0.16em] text-white/80">
            Programme Partner · MC Studio
          </span>
        </div>

        <div class="space-y-5">
          <h1
            class="hero-title font-manrope font-medium leading-[1.15] sm:leading-[1.2] md:leading-tight max-w-[320px] sm:max-w-[500px] md:max-w-[700px] lg:max-w-[820px] text-transparent bg-clip-text bg-[linear-gradient(270deg,#f0bf6c_0%,white_67.308%)] text-[clamp(1.25rem,6vw,1.5rem)] min-[480px]:text-[clamp(1.5rem,5vw,1.75rem)] sm:text-[clamp(2rem,4.5vw,2.25rem)] md:text-[clamp(2.25rem,4vw,2.625rem)] lg:text-[2.625rem] xl:text-[3rem] opacity-0"
            style="filter: blur(20px); transform: translateY(20px)"
          >
            Devenez partenaire de croissance de MC Studio.
          </h1>
          <p
            class="hero-subtitle font-normal leading-[1.6] sm:leading-[1.7] tracking-wide text-white/90 max-w-[560px] text-[clamp(0.8125rem,2.5vw,0.875rem)] sm:text-[clamp(0.875rem,2vw,1rem)] md:text-base lg:text-[1.0625rem] xl:text-[1.1875rem] opacity-0"
            style="filter: blur(15px); transform: translateY(15px)"
          >
            Recommandez des projets stratégiques à MC Studio et contribuez directement à la croissance du studio.
            Pour chaque projet signé, vous touchez
            <span class="text-[var(--color-gold)] font-semibold">10&nbsp;% de commission</span>,
            dans un cadre transparent, structuré et pensé pour durer.
          </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-5">
          ...
          <button
            ...
            type="button"
            class="secondary-btn group flex items-center justify-center gap-2.5 px-5 py-2.5 rounded-xl font-medium text-sm text-[#f0eadb] cursor-pointer transition-all duration-300 no-underline bg-black/50 border border-white/12 hover:bg-black/70"
          ...
          >
            ...
            <UIcon name="i-lucide-arrow-down-right" class="w-4 h-4" />
          </button>
        </div>

        <!-- Highlight card -->
        <div
          class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-[640px]"
        >
          <div
          ...
          >
            <div class="space-y-1">
              ...
              <p class="font-inter text-sm sm:text-[15px] text-white/90">
                Projet signé à
                <span class="font-semibold text-[var(--color-gold)]">15&nbsp;000&nbsp;€</span>
                &nbsp;→ vous touchez
                <span class="font-semibold text-[var(--color-gold)]">1&nbsp;500&nbsp;€</span>.
              </p>
            </div>
          </div>
